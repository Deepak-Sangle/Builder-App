...
const SelectCompanyView = ({navigation}) => {
  ...
  const companiesList = useSelector(state => state.walkInScreen.companies);

  const [company, setCompany] = useState(companiesList);
  //console.log(company);

  let size;
  if (company[company.length - 1].id != 0) size = company.length;
  ...
  return (
    <View style={styles.boxView}>
      <View style={styles.foundCompanyView}>
        <Text style={styles.foundCompanyText}>
          We found {size} companies with the name {name}
        </Text>
      </View>
