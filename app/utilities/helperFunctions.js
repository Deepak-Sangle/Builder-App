...
const getDateArray = (month, year) => {
    ...
    const d = new Date();
    if(month === d.getMonth()+1 && year=== d.getFullYear()) startIndex = d.getDate();
...
const getHoursArray = (date, month, year) => {
    ...
    const d = new Date();
    if(date==d.getDate() && month === d.getMonth()+1 && year=== d.getFullYear()) startIndex = d.getHours()-12;
...
const getMinutesArray = (date, month, year) => {
    let minutes = [], startIndex=0;
    const d = new Date();
    if(date == d.getDate() && month === d.getMonth()+1 && year=== d.getFullYear()) startIndex = d.getMinutes();
...
const getMonthArray = (year) => {
    ...
    if(year === d.getFullYear()) {
        startIndex = d.getMonth()+1; 
    }
    const month = [];
    for(let i=startIndex;i<=12;i++){
        const mm = {label : months[i-1], value : i};
        month.push(mm);
    }
    return month;
}
...
const getAmArray = (date, month, year) => {
    ...
    const d = new Date();
    if(d.getHours()>12 && date === d.getDate() && month === d.getMonth()+1 && year=== d.getFullYear()) return Am.slice(1);
    else return Am;
}
