...
const LogoHeader = props => {
  ...
  const source = props.source
    ? props.source
    : require('../../android/app/src/main/assets/images/Logo.png');
  ...
  const isDescription =
    props.isDescription != undefined ? props.isDescription : false;
    ...
    <View>
      <View style={[styles.container, headingStyle]}>
        {!isHeader && (
          <TouchableOpacity
            onPressOut={goBack}
            activeOpacity={isBack ? 0.5 : 0}
            style={[styles.backView, {opacity: isBack ? 1 : 0}]}>
            <MaterialIcons
              name="arrow-back-ios"
              size={20}
              color={textColor != undefined ? '#FFFFFF' : '#0078E9'}
            />
          </TouchableOpacity>
        )}
        ...
        {isHeader && (
          <View style={styles.companyView}>
            <Text style={[styles.companyText, styles.textStyle]}>
              Builders{'\n'}Broadcast
            </Text>
            {isDescription && (
        ...
        {!isHeader && (
          <TouchableOpacity
            onPressOut={openSideBar}
            activeOpacity={isThreeDot ? 0.5 : 0}
            style={[styles.navigationView, {opacity: isThreeDot ? 1 : 0}]}>
            <Icons
              name="three-bars"
              size={25}
              color={textColor != undefined ? '#FFFFFF' : '#757575'}
            />
          </TouchableOpacity>
        )}
      </View>
      {text && !textTop && (
        <Text
          style={[
            styles.title,
            {color: textColor != undefined ? textColor : '#757575'},
          ]}>
          {text}
        </Text>
      )}
